...
{
    if ($provided === '' || $key === '' || $notificationUrl === '') {
        return false;
    }
...
function squareWebhookClaimEvent(PDO $pdo, array $event): string
{
    ...
    try {
        ...
        $row->execute(['id' => $id]); $status = (string)$row->fetchColumn();
        if ($status === 'failed') {
...
{
    $id = (string)$event['event_id']; $type = (string)$event['type'];
    squareWebhookSetEventStatus($pdo, $id, 'processing');
    try {
        $result = match ($type) {
        ...
        squareWebhookLog("event_id=$id type=$type status=$result");
        return $result;
    } catch (Throwable $e) {
        squareWebhookSetEventStatus($pdo, $id, 'failed', $e->getMessage());
        squareWebhookLog("event_id=$id type=$type status=failed error=" . substr($e->getMessage(), 0, 1000));
        throw $e;
    }
}

function squareWebhookProcessPayment(PDO $pdo, array $event, ?callable $api): string
{
    ...
    if ($api === null) {
        ...
        $api = static fn(string $path): array => squareSyncApiJson($config, 'GET', $path);
    }
    $orderResponse = $api('/v2/orders/' . rawurlencode($orderId)); $order = $orderResponse['order'] ?? null;
...
{
    $variation = trim((string)($line['catalog_object_id'] ?? ''));
    if ($variation === '') { return; }
    $map = $pdo->prepare('SELECT sku_normalized FROM square_catalog_sync WHERE square_variation_id=:id LIMIT 1'); $map->execute(['id' => $variation]);
    $sku = $map->fetchColumn(); $sku = $sku === false ? null : (string)$sku;
    $money = is_array($line['total_money'] ?? null) ? $line['total_money'] : [];
    $uid = trim((string)($line['uid'] ?? $variation));
    ...
    $update = $pdo->prepare("UPDATE intake_items SET status='SOLD', updated_at=datetime('now') WHERE sku_normalized=:sku AND UPPER(TRIM(COALESCE(status,''))) <> 'SOLD'");
...
function squareWebhookProcessRefund(PDO $pdo, array $event): string
{
    ...
    if (!is_array($refund)) { throw new RuntimeException('Refund object missing'); }
    ...
    $where = $paymentId !== '' ? 'square_payment_id=:id' : 'square_order_id=:id';
    $rows = $pdo->prepare("SELECT DISTINCT sku_normalized FROM square_sale_items WHERE $where AND sku_normalized IS NOT NULL"); $rows->execute(['id' => $paymentId !== '' ? $paymentId : $orderId]);
    ...
    foreach ($rows->fetchAll(PDO::FETCH_COLUMN) as $sku) { $update->execute(['status' => $status, 'sku' => $sku]); squareWebhookLog("refund payment_id=$paymentId sku=$sku status=$status changed=" . $update->rowCount()); }
    return 'processed';
}

function squareWebhookProcessInventory(PDO $pdo, array $event): string
{
    $counts = $event['data']['object']['inventory_counts'] ?? $event['data']['object']['inventory_count'] ?? [];
    if (isset($counts['catalog_object_id'])) { $counts = [$counts]; }
    if (!is_array($counts)) { throw new RuntimeException('Inventory count missing'); }
    foreach ($counts as $count) {
        if (!is_array($count)) { continue; }
        $variation = (string)($count['catalog_object_id'] ?? ''); $map = $pdo->prepare('SELECT s.sku_normalized,i.status FROM square_catalog_sync s LEFT JOIN intake_items i ON i.sku_normalized=s.sku_normalized WHERE s.square_variation_id=:id LIMIT 1'); $map->execute(['id' => $variation]); $match = $map->fetch();
        $quantity = (string)($count['quantity'] ?? ''); $note = null;
        if (!$match) { $note = 'No Pinksheet mapping exists'; }
        elseif ((float)$quantity === 0.0 && strtoupper(trim((string)$match['status'])) !== 'SOLD') { $note = 'Square quantity is 0 but Pinksheet is not SOLD'; }
        elseif ((float)$quantity > 0 && strtoupper(trim((string)$match['status'])) === 'SOLD') { $note = 'Square quantity is greater than 0 but Pinksheet is SOLD'; }
        $stmt = $pdo->prepare('INSERT OR IGNORE INTO square_inventory_events(event_id,square_variation_id,sku_normalized,location_id,inventory_state,quantity,occurred_at,reconciliation_status,notes) VALUES(:event,:variation,:sku,:location,:state,:quantity,:occurred,:status,:notes)');
        $stmt->execute(['event' => $event['event_id'], 'variation' => $variation, 'sku' => $match['sku_normalized'] ?? null, 'location' => $count['location_id'] ?? null, 'state' => $count['state'] ?? null, 'quantity' => $quantity, 'occurred' => $count['occurred_at'] ?? null, 'status' => $note ? 'warning' : 'observed', 'notes' => $note]);
        if ($note) { squareWebhookLog("event_id={$event['event_id']} inventory warning=$note variation_id=$variation"); }
    }
    return 'processed';
}
...
{
    $order = $event['data']['object']['order'] ?? []; squareWebhookLog('event_id=' . $event['event_id'] . ' order_id=' . ($order['id'] ?? '') . ' state=' . ($order['state'] ?? ''));
    return 'processed';
}
...
function squareWebhookIsPrivateRequest(): bool
{
    ...
    return $remote === '127.0.0.1' || $remote === '::1' || ($remote !== '' && filter_var($remote, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE) === false);
}
